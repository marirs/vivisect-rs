...
use crate::analysis::{AnalysisModTracker, analyze_function, Analyzer};
use crate::constants::{ARCH_DEFAULT, CB_FUNCVA, ENDIAN_LSB, L_LTYPE, L_SIZE, L_TINFO, L_VA, LOC_IMPORT, LOC_NUMBER, LOC_OP, LOC_POINTER, LOC_STRING, LOC_UNI, LOC_VFTABLE, MM_EXEC, MM_READ, MM_WRITE, REBASE_TYPES, REF_PTR, SEG_FNAME, VASET_ADDRESS, VASET_COMPLEX, VASET_INTEGER, VASET_STRING, VTE_MASK, VWE_ADDFREF, VWE_ADDMMAP, VWE_ADDRELOC, VWE_ADDVASET, VWE_AUTOANALFIN, VWE_COMMENT, VWE_DELRELOC, VWE_SETVASETROW, XR_RTYPE};
...
impl VivWorkspace {
    ...
    pub fn new(confdir: &str, autosave: bool) -> Self{
        ...
        workspace.set_meta("StorageModule", Some("vivisect_rs.storage.basicfile".to_string()));
        ...
        workspace.add_vaset("Emulation Anomalies", vec![("va", VASET_ADDRESS), ("Message", VASET_STRING)]);
        workspace.add_vaset("Bookmarks", vec![("va", VASET_ADDRESS),("Bookmark Name", VASET_STRING)]);
        workspace.add_vaset("DynamicBranches", vec![("va", VASET_ADDRESS),("opcode", VASET_STRING), ("bflags",  VASET_INTEGER)]);
        workspace.add_vaset("SwitchCases", vec![("va", VASET_ADDRESS),("setup_va", VASET_ADDRESS), ("Cases",  VASET_INTEGER)]);
        workspace.add_vaset("PointersFromFile", vec![("va", VASET_ADDRESS),("target", VASET_ADDRESS), ("file",  VASET_STRING), ("comment", VASET_STRING)]);
        workspace.add_vaset("CodeFragments", vec![("va", VASET_ADDRESS),("calls_from", VASET_COMPLEX)]);
        workspace.add_vaset("EnumCodeFunctions", vec![("va", VASET_ADDRESS)]);
        workspace.add_vaset("FuncWrappers", vec![("va", VASET_ADDRESS), ("wrapped_va", VASET_ADDRESS)]);
    ...
    pub fn set_meta(&mut self, meta_name: &str, meta_value: Option<String>) {
        self.metadata.insert(meta_name.to_string(), if meta_value.is_some() { meta_value } else { None });
    }
    ...
    pub fn add_relocation(&mut self, va: i32, r_type: i32, data: Option<Vec<u8>>, mut size: Option<i32>) -> Option<i32>{
        ...
        self.relocations.push((fname, offset, r_type, data.as_ref().cloned().unwrap(), size.as_ref().cloned().unwrap()));
    ...
    pub fn make_pointer(&mut self, va: i32, mut tova: Option<i32>, follow: bool) -> Option<(i32, i32, i32, Vec<(i32, i32)>)> {
        let loctup = self.get_location(va);
        if loctup.is_some() {
            if loctup.as_ref().cloned().unwrap().2 != LOC_POINTER || loctup.as_ref().cloned().unwrap().0 != va {
    ...
    pub fn add_xref(&mut self, from_va: i32, to_va: i32, ref_type: i32, r_flags: i32) {
        ...
        if xr_to.is_none() {
            xr_to = Some(Vec::new());
            self.xrefs_by_to.insert(to_va, xr_to.as_ref().cloned().unwrap());
        }
        if xr_from.is_none() {
            xr_from = Some(Vec::new());
            self.xrefs_by_from.insert(from_va, xr_from.as_ref().cloned().unwrap());
        }
    ...
    pub fn add_location(&mut self, va: i32, size: i32, ltype: i32, tinfo: Option<Vec<(i32, i32)>>) -> (i32, i32, i32, Vec<(i32, i32)>){
    ...
    pub fn set_va_set_row(&mut self, name: &str, row_tup: Vec<i32>) {
        if let Some(defs_rows_tuple) = self.vasets.get(&name.to_string()) {
            self.vasets.insert(name.to_string(), (defs_rows_tuple.clone().0, row_tup));
        } else {
    ...
    pub fn print_discovered_stats(&mut self) {
        let (disc, un_disc, num_xrefs, num_locs, num_funcs, num_blocks, num_ops, num_unis, num_strings, num_numbers, num_pointers, num_vtables) = self.get_discovered_info();
        
        let percentage = if disc > 0 && un_disc > 0 {disc * 100 / (disc/un_disc)} else { 0 };
        info!("Percentage of discovered executable surface area: {}% ({} /{})", percentage, disc, un_disc);
        info!("Xrefs/Blocks/Funcs: ({} / {} / {})", num_xrefs, num_blocks, num_funcs);
        info!("Locs, Ops/Strings/Unicode/Nums/Ptr/Vtables: ({}: {} / {} / {} / {} / {} / {})", num_locs, num_ops, num_strings, num_unis, num_numbers, num_pointers, num_vtables);
    }
    
    /// Returns a tuple of (bytes_with_locations, bytes_without_locations) for all executable maps.
    pub fn get_discovered_info(&mut self) -> (i32, i32, i32, i32, i32, i32, i32, i32, i32, i32, i32, i32) {
        let mut disc = 0;
        let mut un_disc = 0;
        ...
        (disc, un_disc, num_xrefs, num_locs, num_funcs, num_blocks, num_ops, num_unis, num_strings, num_numbers, num_pointers, num_vtables)
    }
    
    pub fn get_xrefs(&self, r_type: Option<i32>) -> Vec<(i32, i32, i32, i32)>{
        if let Some(r_type) = r_type {
            return self.xrefs.iter().filter(|x| x.2 == r_type).map(|x| *x).collect::<Vec<_>>()
        }
        self.xrefs.clone()
    }
    ...
    pub fn get_functions(&mut self) -> Vec<i32> {
        // Vec::new()
        self.funcmeta.iter().map(|x| x.0.clone()).collect::<Vec<_>>()
    }
    ...
    pub fn get_locations(&self, ltype: Option<i32>, linfo: Option<Vec<(i32, i32)>>) -> Vec<(i32, i32, i32, Vec<(i32, i32)>)> {
        if ltype.is_none() {
            return self.loclist.clone()
        }
        if linfo.is_none(){
            return self.loclist.iter().filter(|loc| loc.2 == ltype.as_ref().cloned().unwrap()).map(|x| x.clone()).collect::<Vec<_>>();
        }
        self.loclist.iter().filter(|loc| loc.2 == ltype.as_ref().cloned().unwrap() && loc.3 == linfo.as_ref().cloned().unwrap() ).map(|x| x.clone()).collect::<Vec<_>>()
    }
    ...
    pub fn get_xrefs_to(&self, va: i32, r_type: Option<i32>) -> Vec<(i32, i32, i32, i32)> {
        ...
        xrefs.as_ref().cloned().unwrap().iter().filter(|x_tup| x_tup.2 == r_type.as_ref().cloned().unwrap()).map(|x| *x).collect::<Vec<_>>()
    }
    ...
    pub fn del_code_block(&mut self, va: i32) {
        ...
        self.codeblocks.remove(self.codeblocks.iter().position(|x| *x == cb.as_ref().cloned().unwrap()).unwrap());
        let cb_index = self.codeblocks_by_funcva
        ...
        self.codeblocks_by_funcva
            ...
            .unwrap()
            .remove(cb_index);
        self.blockmap.set_map_lookup(va, cb.as_ref().cloned().unwrap().1, None);
    }
    ...
    pub fn set_function_meta(&mut self, funcva: i32, key: &str, val: i32) {
        ...
        let mcbname = format!("_fmcb_{}", key.split(":").collect::<Vec<_>>().get(0).unwrap());
    }
    ...
    pub fn parse_op_code(&mut self, va: i32) -> Option<OpCode> {
        ...
        let key = (va, arch, b[..16].iter().map(|x| x.clone()).collect::<Vec<_>>());
        let valu = self._op_cache.get(&key).unwrap().clone();
        self._op_cache.insert(key, valu);
        // FIXME Return a properly formed opcode
        Some(OpCode::new(va, 0, "GENERIC", 0, 0, vec![Rc::new( RegisterOper{}), Rc::new(ImmedOper{})]))
    }
            ...
            if count > 0 {
                ...
                if loc.is_some() {
                    if loc.as_ref().unwrap().2 == LOC_UNI {
                        ...
                        return loc.as_ref().cloned().unwrap().0 - (va + count) + loc.as_ref().cloned().unwrap().1;
                    }
                    return -1;
                }
            }
    ...
    pub fn make_unicode(&mut self, va: i32, mut size: Option<i32>) -> (i32, i32, i32, Vec<(i32, i32)>) {
        if size.is_none() {
        ...
        if self.get_name(va, false).is_none() {
            let m = self.read_memory(va, size-1).unwrap()
                .iter()
                .map(|x| 
                ...
                .collect::<Vec<_>>();
            self.make_name(va, format!("wstr_{:?}_{:#0x}", m[..16].to_vec(), va), false, false);
        }
    ...
    pub fn make_function(&mut self, va: i32, meta: Option<HashMap<String, i32>>, arch: i32) {
        ...
        let loc = self.get_location(va);
        if loc.is_some() && !loc.as_ref().cloned().unwrap().3.is_empty() && loc.as_ref().cloned().unwrap().2 == LOC_OP {
    ...
    pub fn get_name(&self, va: i32, smart: bool) -> Option<String> {
        ...
        if base_name.is_none() {
            ...
            base_va = self.get_file_meta(base_name.as_ref().cloned().unwrap().as_str(), "imagebase");
        }
    ...
    pub fn make_name(&mut self, va: i32, mut name: String, file_local: bool, make_unique: bool) -> Option<String> {
        if file_local {
            let seg_tup = self.get_segment(va);
            if seg_tup.is_none() {
                warn!("Failed to find file for {:#0x} ({}) (and file_local != true)", va, name);
            } else { 
        ...
        if old_va.is_some() {
            if !make_unique {
                ...
                debug!("makeName: {} already lives at {:#0x}", name, old_va.as_ref().cloned().unwrap());
                let mut index = 0;
                ...
                while self.va_by_name(new_name.clone()).is_some() {
                    ...
                    debug!("makeName: {} already lives at {:#0x}", new_name.clone(), new_old_va.as_ref().cloned().unwrap());
                    index += 1;
        ...
        if cur_name.is_some() {
            debug!("Replacing {:#0x}: {} -> {}", va, cur_name.as_ref().cloned().unwrap(), name);
    ...
    pub fn uni_string_size(&self, va: i32) -> Option<i32> {
        let (offset, bytes) = self.get_byte_def(va);
        let foff = String::from_utf8(bytes).unwrap().find("\x00\x00").map(|x| x as i32);
        if foff.is_some(){
            return foff;
        }
    ...
    pub fn get_substrings(&self, va: i32, size: i32, r_type: i32) -> Option<Vec<(i32, i32)>> {
        ...
        for offs in (va..end).step_by(1) {
            ...
            if loc.is_some() && loc.as_ref().cloned().unwrap().2 == LOC_STRING && loc.as_ref().cloned().unwrap().0 > va {
                subs.push((loc.as_ref().cloned().unwrap().0, loc.as_ref().cloned().unwrap().1));
                if loc.as_ref().cloned().unwrap().3.len() > 0 {
    ...
    pub fn get_str_info(&self, mut va: i32, mut size: i32, subs: Option<Vec<(i32, i32)>>) -> (i32, i32, Option<Vec<(i32, i32)>>){
    ...
    pub fn add_vaset(&mut self, name: &str, defs: Vec<(&str, i32)>) {
        self.vasets.insert(name.to_string(), (Some(defs.iter().map(|(name, val)| (name.to_string(), *val)).collect()), vec![]));
    }
    ...
    pub fn load_from_file(&mut self, filename: &str, mut fmt_name: Option<String>, base_addr: Option<i32>) -> String{
...
impl Memory for VivWorkspace {
        ...
        for (m_va, m_max_va, m_map, m_bytes) in self._map_defs.clone() {
            if m_va <= va && va < m_max_va {
                ...
                if m_perms & MM_READ == 0 {
                    panic!("Bad Memory Read (no read permission): {:#0x}: {:#0x} ", va, size);
                }
                let offset = va - m_va;
                let max_read_len = m_size - offset;
                if size > max_read_len {
                    let mut new_bytes = Vec::new();
                    new_bytes.append(&mut m_bytes[offset as usize..].iter().map(|x| x.clone()).collect::<Vec<_>>());
                    new_bytes.append(&mut self.read_memory(m_va + m_size, size - max_read_len).unwrap());
                    return Some(new_bytes)
                }
            }
        }
        panic!("Bad memory read (invalid memory address): {:#0x}: {:#0x}", va, size);
    }
        ...
        for mut mapdef in self._map_defs.clone() {
            ...
            if mva <= va && va < mmaxva {
                ...
                if mperms & MM_WRITE == 0 {
                    panic!("Bad Memory Write (no read permission): {:#0x}: {:#0x} ", va, bytes_len);
                }
                let offset = va - mva;
                let max_write_len = msize - offset;
                if bytes_len > max_write_len {
                    let mut new_bytes = Vec::new();
                    new_bytes.append(&mut mbytes[..offset as usize].iter().map(|x| x.clone()).collect::<Vec<_>>());
                    new_bytes.append(&mut bytes[..max_write_len as usize].iter().map(|x| x.clone()).collect::<Vec<_>>());
                    mapdef.3 = new_bytes; 
                    self.write_memory(mva + msize, bytes[max_write_len as usize..].iter().map(|x| x.clone()).collect::<Vec<_>>());
                } else {
                    let mut new_bytes = Vec::new();
                    new_bytes.append(&mut mbytes[..offset as usize].iter().map(|x| x.clone()).collect::<Vec<_>>());
                    new_bytes.append(&mut bytes.clone());
                    new_bytes.append(&mut mbytes[(offset + bytes_len) as usize..].iter().map(|x| x.clone()).collect::<Vec<_>>());
                    mapdef.3 = new_bytes;
                }
            }
        }
        panic!("Bad memory read (invalid memory address): {:#0x}: {:#0x}", va, bytes_len);
    }
    ...
    fn add_memory_map(&mut self, map_va: i32, perms: i32, fname: &str, mut bytes: Vec<u8>, alignment: Option<i32>) -> i32 {
        debug!("Map VA: {} Perms: {}, fname: {}, bytes: {}", map_va, perms, fname, bytes.len());
