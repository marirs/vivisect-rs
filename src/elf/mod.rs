...
//!
//! To access the exact 32-bit or 64-bit versions, use , assets., for the various 32/64-bit structs.
//!
//! # Example
//!
//! ```rust
...
//!
//!
//! You are still free to use the specific 32-bit or 64-bit versions by accessing them through `elf::elf64`, assets., but you will have to parse and/or construct the various components yourself.
...
if_sylvan! {
    ...
    pub struct Elf<'a> {
        ...
        /// The dynamic relocation entries (strings, copy-data, assets.) with an addend
...
mod tests {
    ...
    #[test]
    fn parse_crt1_64bit() {
        let crt1: Vec<u8> = include!("../../assets/crt1.rs");
    ...
    #[test]
    fn parse_crt1_32bit() {
        let crt1: Vec<u8> = include!("../../assets/crt132.rs");
        match Elf::parse(&crt1) {
            Ok(binary) => {
                assert!(!binary.is_64);
                assert!(!binary.is_lib);
